...
#
#set -x
tso alloc dsn\("'"${TESTHLQ}".MVSCMD.ADRDSSU.DARV1'"\) recfm\(u\) lrecl\(0\) blksize\(32760\) dsorg\(ps\) dsntype\(basic\) catalog tracks space\(10000\) >/dev/null 2>&1

(
 . setcc adrdssuCreateViaDataset;
 mvscmdauth -v --pgm=ADRDSSU --args='' --archive=${TESTHLQ}.mvscmd.adrdssu.darv1,old --sysin=${TESTHLQ}.mvscmd.adrdssu.cmd --sysprint=* | awk '!/1PAGE 0001|ADR109I|ADR006I|ADR801I|ADR006I|ADR013I|ADR012I/'
 . unsetcc

)

(tsocmd delete "'"${TESTHLQ}".MVSCMD.ADRDSSU.DARV1'" >/dev/null 2>&1) 

tso alloc dsn\("'"${TESTHLQ}".MVSCMD.ADRDSSU.DARV2'"\) recfm\(u\) lrecl\(0\) blksize\(32760\) dsorg\(ps\) dsntype\(basic\) catalog tracks space\(10000\) >/dev/null 2>&1
...
(tsocmd delete "'"${TESTHLQ}".MVSCMD.ADRDSSU.DARV2'" >/dev/null 2>&1)
