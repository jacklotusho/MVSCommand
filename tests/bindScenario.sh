...
. setcc bindProgram;
mvscmd -v --pgm="HEWL" --args="MAP" --syslin=${TESTHLQ}.mvscmd.bind.obj\(bind\) --syslmod=${TESTHLQ}.mvscmd.bind.load\(bind\) --sysprint=* --syslib=${LEHLQ}.SCEELKED:${LEHLQ}.SCEELKEX:${LEHLQ}.SCEECPP | 
  awk '!/^1z\/OS V2 R2 BINDER|BATCH EMULATOR|TIME OF SAVE/' | 
  /bin/sed -e 's/ \+VOLUME SERIAL \+[^ ]\+/ VOLUME SERIAL ******/' | 
  /bin/sed -e 's/SYS[0-9]* /SYSxxxx /g' |
  /bin/sed -e 's/SYS[0-9]*=/SYSxxxx=/g'
