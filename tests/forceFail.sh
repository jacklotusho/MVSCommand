...
(export FAIL_HFSAllocation=1; . setcc ffHFSAllocation; mvscmd --mydd=/tmp; echo $?)
(export FAIL_StdinAllocation=1; . setcc ffStdinAllocation; echo 'hi there' | mvscmd --mydd=stdin; echo $?)
(export FAIL_PrintDatasetToConsole=1; . setcc ffPrtToStdout;  mvscmd --mydd=* 2>/tmp/$$.out; echo $?; cat /tmp/$$.out | sed -e 's/dataset [A-Z0-9\.]* to console/dataset xxx to console/g'; . unsetcc;)
